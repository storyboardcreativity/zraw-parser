    ...
    ConversionResult process_zraw_old_raw(std::atomic<bool>& itsTimeToStopOkay, IConsoleOutput &console, IProgressBar& progressBar, std::istream &f_in, TinyMovFile &mov, std::string &output_path)
    {
		for (int i = 0; i < mov.Tracks().size(); ++i)
		{
			...
			for (int i = 0; i < track.Media().Info().SampleSizes().size(); ++i)
			{
				...
				progressBar.ChangePercent(i * (100.0 / track.Media().Info().SampleSizes().size()));
				...
				std::string output_dng_path = output_path + "/track_" + std::to_string(i) + "_" + std::to_string(i) + ".dng";

				//#define _ZRAWF_EXPORT
#ifdef _ZRAWF_EXPORT

				std::string output_zrawf_path = output_path + "/track_" + std::to_string(track_index) + "_" + std::to_string(i) + ".zrawf";
				...
				while (!th_found)
				{
					for (int th_index = 0; th_index < 1; ++th_index)
					{
    ...
    ConversionResult process_zraw_XORred(std::atomic<bool>& itsTimeToStopOkay, IConsoleOutput &console, IProgressBar& progressBar, std::istream &f_in, TinyMovFile &mov, std::string &output_path)
    {
		...
		writer.ChunkHandler([&](TinyMovTrack& track, uint64_t chunk_offset, std::vector<uint8_t>& chunk_data)
		{
			...
			if (pack_count)
			{
				...
				for (int g = 0; g < chunk_data.size(); ++g)
